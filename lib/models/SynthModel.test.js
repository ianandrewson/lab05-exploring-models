...
describe('SynthModel tests', () => {
  ...
  it('should have at least one amp', () => {
    ...
    const { errors } = synth.validateSync();
    expect(errors.amps.message).toEqual('Path `amps` (0) is less than minimum allowed value (1).');
  });
  it('should have a valid filter', () => {
    ...
    const { errors } = synth.validateSync();
    expect(errors.filters.message).toEqual('`midpass` is not a valid enum value for path `filters`.');
  });
